...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  {
	    /* --- PAPILDOMA DALIS (ADC) --- */
	    /* UŽKOMENTUOK ŠĮ BLOKĄ, KAI NAUDOJI TIM2 INPUT CAPTURE */
	    /*
	    HAL_ADC_Start(&hadc1);
	    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK) {
	        adc_val = HAL_ADC_GetValue(&hadc1);
	        // Jei įtampa žema (mygtukas nepaspaustas) - žalias
	        if (adc_val < 1000) {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM4) {
        ...
        // --- PG13: keičia būseną kas 2s (2000ms / 10ms = 200 žingsnių) ---
        // Šitą dalį užkomentuok, kai veikia 2-oji užduotis (nes abu valdo tuos pačius LED)
      /*  pg13_timer++;
        if (pg13_timer >= 200) {
        ...
        // --- PG14: 4s periodas, 3.5s ON, 0.5s OFF ---
        pg14_timer++;
        ...
        // --- 2 UŽDUOTIES INDIKACIJA (mirktelėjimas matuojant) ----------------------------------------------------------------------------------
        if (is_measuring) {
            blink_timer++;
            if (blink_timer % 100 < 10) HAL_GPIO_WritePin(GPIOG, GPIO_PIN_14, GPIO_PIN_SET);
            else HAL_GPIO_WritePin(GPIOG, GPIO_PIN_14, GPIO_PIN_RESET);
        }
    }
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) {
        if (capture_idx == 0) { // Pirmas paspaudimas
            cap1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
            capture_idx = 1;
            is_measuring = 1;
            HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13 | GPIO_PIN_14, GPIO_PIN_RESET);
        }
        else { // Antras paspaudimas
            cap2 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_1);
            is_measuring = 0;
            capture_idx = 0;

            // 1. Paskaičiuojam bendrą praėjusį laiką sekundėmis
            diff_time = (float)(cap2 - cap1) / 8571.0f;

            // 2. CIKLIŠKUMO LOGIKA:
            // Jei laikas viršija 12 sekundžių, atimame po 12, kol gausime skaičių nuo 0 iki 12.
            // Pvz., jei diff_time yra 15, gausime 3. Jei 25, gausime 1.
            while (diff_time > 12.0f) {
                diff_time -= 12.0f;
            }

            // 3. Išjungiam viską ir parodom rezultatą pagal tavo lentelę (0-12s lange)
            HAL_GPIO_WritePin(GPIOG, GPIO_PIN_13 | GPIO_PIN_14, GPIO_PIN_RESET);
